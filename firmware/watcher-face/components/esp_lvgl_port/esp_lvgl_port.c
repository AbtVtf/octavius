...
static void lvgl_port_encoder_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    ...
    ESP_LOGI(TAG, "Encoder Read: invd=%d, event=%d, last_v=%d", invd, event, last_v); // Debug print

    if (last_v != invd) // Changed from XOR to direct comparison
    {
        last_v = invd;
        data->enc_diff = (KNOB_LEFT == event) ? (-1) : ((KNOB_RIGHT == event) ? (1) : (0));
        ESP_LOGI(TAG, "Encoder Diff: %d", data->enc_diff); // Debug print
    }
    else
    {
        data->enc_diff = 0;
